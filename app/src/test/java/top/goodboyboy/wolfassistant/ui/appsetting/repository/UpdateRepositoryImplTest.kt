...
class UpdateRepositoryImplTest {
     ...
     */
    @Test
    fun `checkUpdate returns Success when new version is available`() = runTest {
        // 准备数据：当前版本为 1.0.0，最新版本为 1.1.0
        val currentVersion = "1.0.0"
        val latestVersion = "1.1.0"
        val versionInfo = VersionInfo(version = latestVersion, htmlUrl = "url", isPrerelease = false, body = "update")

        // 模拟 GitHubDataSource 返回成功结果
        coEvery { gitHubDataSource.checkUpdateInfo() } returns GitHubDataSource.VersionDataResult.Success(versionInfo)
     ...
     */
    @Test
    fun `checkUpdate returns NOUpdate when current version is newer`() = runTest {
     ...
     */
    @Test
    fun `checkUpdate returns NOUpdate when versions are equal`() = runTest {
     ...
     */
    @Test
    fun `checkUpdate returns Error when data source fails`() = runTest {
        // 准备数据
        val currentVersion = "1.0.0"
     ...
     */
    @Test
    fun `checkUpdate handles pre-release versions correctly`() = runTest {
        // Case 1: beta < rc (Update available)
        var currentVersion = "1.0.0-beta"
        var latestVersion = "1.0.0-rc"
        var versionInfo = VersionInfo(version = latestVersion, htmlUrl = "url", isPrerelease = true, body = "update")
        coEvery { gitHubDataSource.checkUpdateInfo() } returns GitHubDataSource.VersionDataResult.Success(versionInfo)
     ...
     */
    @Test
    fun `checkUpdate handles pre-release version numbers correctly`() = runTest {
     ...
     */
    @Test
    fun `checkUpdate prioritizes core version over suffix`() = runTest {
        // Case 1: 1.0.0 < 1.1.0-alpha (Update available)
        val currentVersion = "1.0.0"
        val latestVersion = "1.1.0-alpha"
        val versionInfo = VersionInfo(version = latestVersion, htmlUrl = "url", isPrerelease = true, body = "update")
        coEvery { gitHubDataSource.checkUpdateInfo() } returns GitHubDataSource.VersionDataResult.Success(versionInfo)
        val result = repository.checkUpdate(currentVersion)
