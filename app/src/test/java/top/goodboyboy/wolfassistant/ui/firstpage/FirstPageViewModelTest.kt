...
class FirstPageViewModelTest {
    ...
    private fun generateTestJWT(expirationTimeSeconds: Long? = null): String {
        ...
        val headerEncoded = java.util.Base64.getUrlEncoder().withoutPadding()
            .encodeToString(header.toByteArray(StandardCharsets.UTF_8))
        val payloadEncoded = java.util.Base64.getUrlEncoder().withoutPadding()
    ...
    @Test
    fun `init should set hasAccessToken to false and loadState to Success when token is empty`() = runTest(testDispatcher) {
        // Given: 空token
    ...
    @Test
    fun `init should set hasAccessToken to true when token exists and not expired`() = runTest(testDispatcher) {
    ...
    @Test
    fun `init should detect expired token and set hasTokenExpired to true`() = runTest(testDispatcher) {
    ...
    @Test
    fun `init should set loadState to Failed when exception occurs`() = runTest(testDispatcher) {
    ...
    @Test
    fun `init should initialize global config with SSL and IPv4 settings`() = runTest(testDispatcher) {
        // Given: 配置SSL和IPv4设置
        val disableSSL = true
        val onlyIPv4 = true
        coEvery { settingsRepository.accessTokenFlow } returns flowOf("")
