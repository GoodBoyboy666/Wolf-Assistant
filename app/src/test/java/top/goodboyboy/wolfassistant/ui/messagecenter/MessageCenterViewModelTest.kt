...
class MessageCenterViewModelTest {
     ...
     */
    @Test
    fun `accessToken empty should use createErrorFlow`() = runTest(testDispatcher) {
        // Given
        ...
        // When
        val flow = MessageCenterViewModel(messageRepository, settingsRepository, application)
     ...
     */
    @Test
    fun `getAppID failed should use createErrorFlow`() = runTest(testDispatcher) {
        // Given
        every { settingsRepository.accessTokenFlow } returns flowOf("token")
        coEvery { messageRepository.getAppID("token") } returns MessageRepository.AppIDData.Failed(top.goodboyboy.wolfassistant.common.Failure.IOError("err", null))
     ...
     */
    @Test
    fun `category 0 should use online appid and call getMessages`() = runTest(testDispatcher) {
        // Given
        every { settingsRepository.accessTokenFlow } returns flowOf("token")
        val pd = TestPagingDataFactory.create(MessageItem("title", "author", "time", "content"))
        coEvery { messageRepository.getAppID("token") } returns MessageRepository.AppIDData.Success(listOf("a0", "a1"))
     ...
     */
    @Test
    fun `category 1 should use data index 1 appid and call getMessages`() = runTest(testDispatcher) {
        // Given
        every { settingsRepository.accessTokenFlow } returns flowOf("token")
        val pd = TestPagingDataFactory.create()
        coEvery { messageRepository.getAppID("token") } returns MessageRepository.AppIDData.Success(listOf("a0", "a1", "a2"))
     ...
     */
    @Test
    fun `unknown category should pass empty appid to getMessages`() = runTest(testDispatcher) {
        // Given
        every { settingsRepository.accessTokenFlow } returns flowOf("token")
        val pd = TestPagingDataFactory.create()
     ...
     */
    @Test
    fun `getMessagePagingFlow should cache flow per category`() = runTest(testDispatcher) {
        // Given
        every { settingsRepository.accessTokenFlow } returns flowOf("token")
        coEvery { messageRepository.getAppID("token") } returns MessageRepository.AppIDData.Success(listOf("a0", "a1"))
     ...
     */
    @Test
    fun `accessToken change should trigger flatMapLatest`() = runTest(testDispatcher) {
        ...
        coEvery { messageRepository.getAppID(any()) } returns MessageRepository.AppIDData.Success(listOf("a0", "a1"))
        coEvery { messageRepository.getMessages(any(), any()) } returns flowOf(TestPagingDataFactory.create())
