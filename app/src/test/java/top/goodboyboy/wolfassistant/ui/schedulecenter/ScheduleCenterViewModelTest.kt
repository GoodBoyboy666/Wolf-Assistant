...
class ScheduleCenterViewModelTest {
     ...
     */
    @Test
    fun `setFirstAndLastDay should update firstDay and lastDay flows`() = runTest(testDispatcher) {
     ...
     */
    @Test
    fun `loadScheduleList should fail when dates are null`() = runTest(testDispatcher) {
        viewModel = ScheduleCenterViewModel(scheduleCenterRepository, settingsRepository, globalEventBus)
     ...
     */
    @Test
    fun `loadScheduleList success should update scheduleList and set Success state`() = runTest(testDispatcher) {
        viewModel = ScheduleCenterViewModel(scheduleCenterRepository, settingsRepository, globalEventBus)
     ...
     */
    @Test
    fun `loadScheduleList failure from repository should set Failed state and emit error message`() = runTest(testDispatcher) {
        viewModel = ScheduleCenterViewModel(scheduleCenterRepository, settingsRepository, globalEventBus)
     ...
     */
    @Test
    fun `cleanCache should call repository cleanScheduleCache`() = runTest(testDispatcher) {
        viewModel = ScheduleCenterViewModel(scheduleCenterRepository, settingsRepository, globalEventBus)
